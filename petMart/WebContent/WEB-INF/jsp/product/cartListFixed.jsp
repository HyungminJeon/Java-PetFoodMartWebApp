...
function purchase() {
	 var checkboxes = document.getElementsByName('check');
	 var id = document.getElementById('id').value; // userId
	 var paySum = 0; // 체크된 물품들의 합계 가격
	 
	 for (var i=0; i < checkboxes.length; i++) {
         if ( $(checkboxes[i]).is(':checked') ) { 
             var itemCode = checkboxes[i].value; // == ${vo.itemCode }
             paySum += parseInt($('#'+itemCode+'_rowSum').val()); // == $('#${vo.itemCode }_rowSum').val()
         }
     }
				...
				<c:forEach items="${userCartList }" var="vo">
					...
					<td><input type="hidden" id="id" value="${vo.userId }"></td>
						...
						<c:if test="${vo.sale eq 'Y'}">
							<td>${vo.salePrice }</td>
							<c:set var="sum" value="${sum + (vo.salePrice*vo.itemQty) }"></c:set>
							<input type="hidden" value="${(vo.salePrice*vo.itemQty) }" id="${vo.itemCode }_rowSum">
						</c:if>
						<c:if test="${vo.sale ne 'Y'}">
							<c:set var="sum" value="${sum + (vo.price*vo.itemQty) }"></c:set>
							<input type="hidden" value="${(vo.price*vo.itemQty) }" id="${vo.itemCode }_rowSum">
							<td></td>
						</c:if>
						<td>${vo.itemQty }</td>
